 ...
 * @author home
 */
public class VCrearFichaConvenio extends javax.swing.JPanel {
    ...
    private static Convenio stConvenio = new Convenio();
    Boton boton = new Boton();
    /**
     * Creates new form VNuevaFicha
     */
    public VCrearFichaConvenio() throws SQLException, ClassNotFoundException {
        
        initComponents();
        stInventario = new Inventario();
        stConvenio = GV.getConvenio();
        if(stConvenio == null){
            OptionPane.showMsg("Registro seleccionado no existe", "Debe seleccionar un convenio válido", 2);
            boton.convenios();
        }
        try {
            stInventario = (Inventario)load.get(GV.inventarioName(), 0, new Inventario());
        } catch (InstantiationException ex) {
            Logger.getLogger(VCrearFichaConvenio.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            Logger.getLogger(VCrearFichaConvenio.class.getName()).log(Level.SEVERE, null, ex);
        }
        ContentAdmin.lblTitle.setText("Nueva ficha con convenio: "+stConvenio.getNombre());
        validateDataTemp();
        load();
    ...
    private void calcularSaldo() {
        ...
        GV.getFichaConvenio().setDescuento(descuento);
    ...
    private void cargarDatos() throws SQLException, ClassNotFoundException {
        ...
        resetTxtCliente();
        Ficha ficha = GV.getFichaConvenio();
        ...
        GV.getFichaConvenio().setDescuento(0);
    ...
    private void calcularTotal() {
        ...
        total = total + ((total * stConvenio.getPorcentajeAdicion())/100);
    ...
    private void comprobarDatosFicha(){
        ...
        if(txtFecha.getDate() == null || !lugarEntregaValido()){
            ...
            GV.getFichaConvenio().setConvenio(stConvenio);
            GV.getFichaConvenio().setFecha(new Date());
            GV.getFichaConvenio().setFechaEntrega(txtFecha.getDate());
            GV.getFichaConvenio().setLugarEntrega(txtEntrega.getText());
            GV.getFichaConvenio().setUser(GV.user());
            GV.getFichaConvenio().setObservacion(txtObs.getText());
            if(!hourIsValid()){
                                        ...
                                        if(!valorTotalValido()){
                                            ...
                                            if(someGlassAndOneInStock()){
                                                ...
                                                if(!TipoPagoValido()){
                                                    ...
                                                    asigAllDatas();
                                                    if(GV.getFichaConvenio().getSaldo()<0){
        ...
        if(!GV.getStr(txtRutCliente.getText()).isEmpty() || 
            ...
            if(!nombre.isEmpty() && nombre.length() > 3){
                if((!tel1.isEmpty() && tel1.length() >  8) ||
                    ...
                    GV.getFichaConvenio().setCliente(stCliente);
                    return true;
                }
            }
        }
...
//                resetTxtCliente();
    ...
    private boolean lugarEntregaValido() {
        ...
        if(largo > 4 && largo < 45){
            GV.getFichaConvenio().setLugarEntrega(GV.getStr(txtEntrega.getText()));
            return true;
        }
        return false;
    }
    ...
    private boolean validaEspecialista() {
        stDoctor = null;
        GV.getFichaConvenio().setDoctor(null);
        ...
        txtDoctor.setForeground(negro);
        GV.getFichaConvenio().setDoctor(stDoctor);
        return true;
    }
    ...
    private void asigAllDatas() {
        ...
        GV.getFichaConvenio().setLejos(stArmazonLejos);
        GV.getFichaConvenio().setCerca(stArmazonCerca);
        
        GV.getFichaConvenio().setObservacion(txtObs.getText());
        GV.getFichaConvenio().setDescuento(GV.strToNumber(txtDescuento.getText()));
        GV.getFichaConvenio().setValorTotal(GV.strToNumber(txtTotal.getText()));
        GV.getFichaConvenio().setSaldo(GV.strToNumber(txtSaldo.getText()));
    }

    private boolean institucionValida() {
        String ins = GV.getStr(txtInstitucion.getText());
        stInstitucion = null;
        GV.getFichaConvenio().setInstitucion(null);
        if(ins.isEmpty()){
        ...
        GV.getFichaConvenio().setInstitucion(stInstitucion);
        txtInstitucion.setForeground(negro);
        return true;
    }
...
//         validacion de estados de ficha
        int estado = (GV.getFichaConvenio().getSaldo() == 0)? 2:1;//2=pagado,1=pendiente
        commitSpinner();
        int abono = (int) txtAbono.getValue();
        ...
        try {
            if(load.get(load.getCurrentCod(GV.getFichaConvenio()), 0, GV.getFichaConvenio()) != null){
                OptionPane.showMsg("No se puede crear registro", "Existe un conflicto con el identificador generado\n"
                        ...
                        + "Identificador defectuoso:"+load.getCurrentCod(GV.getFichaConvenio()), 3);
                return;
            }
        } catch (SQLException | ClassNotFoundException | InstantiationException | IllegalAccessException ex) {
            OptionPane.showMsg("No se puede crear registro", "No se pudo validar el identificador generado\n"
                        + "Debe ponerse en contacto con su proveedor.\n\n"
                        + "Identificador defectuoso:"+load.getCurrentCod(GV.getFichaConvenio())+"\n"
                        + "Error: "+ex.getMessage(), 3);
            return;
        }
        GV.getFichaConvenio().setCod(load.getCurrentCod(GV.getFichaConvenio()));
        /*
        ...
        */
        ...
        //GV.getFichaConvenio().setValorTotal(valorTotal);
        GV.getFichaConvenio().setSaldo(saldo);
        GV.getFichaConvenio().setEstado(estado);
        
        if(GV.getFichaConvenio().getCerca() != null || !GV.getFichaConvenio().getCerca().getMarca().isEmpty()){
            GV.getFichaConvenio().getCerca().setIdFicha(GV.getFichaConvenio().getCod());
        }
        if(GV.getFichaConvenio().getLejos() != null || !GV.getFichaConvenio().getLejos().getMarca().isEmpty()){
            GV.getFichaConvenio().getLejos().setIdFicha(GV.getFichaConvenio().getCod());
        }
        if(abono > 0){
            hp = new HistorialPago(null, new Date(), abono, stTipoPago.getId(), GV.getFichaConvenio().getCod(),1, null, 0);
        }
        
        load.createFicha(GV.getFichaConvenio(),hp);
    }
    ...
    private void validateDataTemp() {
        if(GV.getFichaConvenio().getFecha() != null){
